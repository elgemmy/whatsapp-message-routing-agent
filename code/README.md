...
Luna routing uses OpenRouter reasoning effort `max`. Run manifests bind that setting, the 4,000-token output ceiling, prompt version, and STT model, so a resume rejects configuration drift. The ceiling was measured rather than guessed: one image smoke exhausted 2,000 tokens with `finishReason=length`, while the valid smoke responses stayed below it. Prompt `routing-v2` asks for a complete short reason and allows enough schema headroom to avoid the prior 240-character truncation boundary.
