...
Routing defaults to OpenRouter reasoning effort `high`. Override it with `--reasoning-effort`; run manifests bind that setting, the 8,000-token output ceiling, prompt version, and STT model, so a resume rejects configuration drift. Prompt `routing-v4-final` is self-contained in `src/routing.ts`: it restores the stronger measured v4 type/payment policy and keeps narrow action safety/tie-break corrections. It also enforces input grounding, a 200-character reason limit, and up to 12 materially useful evidence IDs without padding.
