...
- Raise Luna's output ceiling from 2,000 to 4,000 for the durable baseline. In the six-case gate, `sample_msg_048` consumed exactly 2,000 output tokens and ended with `finishReason=length`; the failure was therefore budget exhaustion, not evidence that Luna could not understand the image. The new bound is also manifest-pinned and must be evaluated before considering a separate image model.
